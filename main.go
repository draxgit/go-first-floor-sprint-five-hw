...
type CaloriesCalculator interface {
	...
	Calories() float64         // метод для расчета количества потраченных калорий
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	durationInHours := s.Duration.Hours()
	speed := (s.meanSpeed() * SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * durationInHours
	return speed
}
...
func ReadData(training CaloriesCalculator) string {
	...
	info.Calories = calories // Дополняем поле Calories в структуре InfoMessage
